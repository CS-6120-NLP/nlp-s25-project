...
def retriever1_manual(query):
    service = RetrievalService()
    return service.retrieve_from_manual_catalog(query)


def retriever2_semantic(query: str):
    service = RetrievalService()
    return service.retrieve_from_semantic_cluster(query)


def retriever3_page_chunks(query: str):
    service = RetrievalService()
    return service.retrieve_from_page_chunks(query)


def retriever4_hyde(query: str):
    service = RetrievalService()
    return service.retrieve_with_hyde(query)


def hybrid_query_model(query: str):
    service = RetrievalService()
    return service.hybrid_retrieval(query)


def final_retriever(query: str):
    service = RetrievalService()
